...
const FetchGoogleCalendar = function ({
  ...
  const loadEvents = () => gapi.client.calendar.events.list({
    calendarId: 'primary',
   ...
   */
  const handleClick = () => gapi.auth2.getAuthInstance().signIn()
    .catch((err) => {
    ...
    .catch((err) => {
      ...
      console.log('Error logging user out of Google:', err);
    })
    .then(() => {
      setCalendarDataIsLoading(false);
  ...
  /**
   * Fetch the events from the user's Google Calendar and pass
   * them to the storeGoogleAvailability function from props
   */
  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.googleFetchHelpText}>
        <b>Plan.it</b>
        {' '}
        can fill in your availability automatically from your Google Calendar.
        You will be able to adjust your availability before submitting.
      </p>
    </div>
  );
};
